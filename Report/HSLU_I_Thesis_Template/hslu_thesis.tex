...
\author{Laura Livers}                            % author name
\city{Zug (Switzerland)}                    % author's place of origin
\title{so few data, so much AI}                            % thesis title
\subtitle{\large Synthetic Data for Rare-Event Detection}                      % thesis subtitle

\date{2026}                                     % the year when the thesis was written (used in titlepage)
\defensedate{May 27th, 2026}                % the date of the private defense
\defencelocation{Rotkreuz}                       % location of defence
\extexpert{Expert Name}                         % name of external expert
\indpartner{Ascento AG}                       % name of industry partner
\studyprogram{Artificial Intelligence and Machine Learning}                    % name of study program: Business Information Technology, International IT Management, ...
...
\supervisor{Prof. Dr. Jan Svoboda}             % name of supervisor
\dean{Prof. Dr. Name Surname}                   % name of faculty dean

\acknowledgments{NONO. not at all. Thanks to my family, relatives and firends for all the support given to finish this thesis.}
...
% IX - List of Abbreviations, Figures, Tables, AI Tools, Formulas
\listoffigures
\listoftables
% print list of acronyms and glossary
\printnoidxglossaries
\input{chapters/09_lists}
...
% 1 - Problem, Research Question, Vision
\input{chapters/01_problem}

% 2 - State of the Art
\input{chapters/02_state_of_the_art}

% 3 - Ideas and Concepts
\input{chapters/03_ideas_and_concepts}

% 4 - Methods
\input{chapters/04_methods}

% 5 - Realization
\input{chapters/05_realization}
